...
void TCPSender::fill_window() {
    ...
    start:
    if (_syn && _stream.buffer_empty() && !_stream.input_ended()) return;

    // FIN-SENT
    if (_stream.eof() && next_seqno_absolute() == _stream.bytes_written() + 2 && bytes_in_flight() > 0) {
...
//    if (_fin && _stream.buffer_empty()) return;
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    // 这样一想，直接把确认号之前所有保留的报文段删除即可
    uint64_t new_ackno = unwrap(ackno, _isn, _next_seqno);
//    if (new_ackno <= _last_ackno) return;
